...
class MongoVectorDBStorage(BaseVectorStorage):
        ...
        try:
            ...
            for index in indexes:
                if index["name"] == text_index_name:
                    logger.info(
                        f"[{self.workspace}] text search index {text_index_name} already exists"
                    )
                    return
            ...
            await self._data.create_search_index(search_index_model)
            logger.info(
                f"[{self.workspace}] Text search index {text_index_name} created successfully."
            )
    ...
    async def hybrid_query(
        ...
        try:
            ...
            formatted_results = [
                {
                    **doc,
                    ...
                    "search_type": "hybrid_rrf" if use_rank_fusion else "hybrid_score",
            ...
            if formatted_results:
                logger.info(
                    f"[HYBRID_SEARCH] Top result score: {formatted_results[0].get('distance', 0):.4f}"
                )
    ...
    async def _manual_rrf_search(
        ...
        Args:
            ...
            top_k: Number of results to return
            rrf_k: RRF constant (default 60, MongoDB's default)
        ...
        # RRF formula: score(d) = sum(1 / (k + rank_i)) for each ranking list
        ...
        for rank, doc in enumerate(text_results):
            ...
            if doc_id not in doc_scores:
                ...
                doc_scores[doc_id]["text_rank"] = rank + 1
            doc_scores[doc_id]["rrf_score"] += rrf_contribution
            ...
            formatted_results.append(
                {
                    **doc,
                    "id": doc["_id"],
                    "distance": item["rrf_score"],
                    "search_type": "manual_rrf",
        ...
        if formatted_results:
            ...
            logger.info(
                f"[MANUAL_RRF] Top result: score={top_result['distance']:.4f}, "
                f"vector_rank={top_result['vector_rank']}, text_rank={top_result['text_rank']}"
            )

        logger.info(f"[MANUAL_RRF] Returned {len(formatted_results)} fused results")
        return formatted_results
